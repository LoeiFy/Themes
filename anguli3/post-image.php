...
<article class="post_image post" style="height: <?php echo $image_url[2] ?>px;">

    <a style="width: 300px; height: <?php echo $image_url[2] ?>px;" class="more" href="<?php the_permalink() ?>" title="<?php the_title(); ?>">

		<img width="300" height="<?php echo $image_url[2] ?>" src="<?php echo $image_url[0] ?>"/>
	
	</a>

	<a class="more overlay" href="<?php the_permalink() ?>" title="<?php the_title(); ?>">
...
<?php } else { ?>
	...
	<div class="image_imgs">

		<div class="img_grid center">
        <?php foreach ( $attachments as $attachment ) { ?>
        <?php $image_attributes = wp_get_attachment_image_src($attachment->ID,'thumbnail');?>
			<a class="postimgs" style="width: 280px; height: <?php echo $image_attributes[2]*14/15 ?>px;" href="<?php echo wp_get_attachment_url( $attachment->ID , false ); ?>"><img width="280" height="<?php echo $image_attributes[2]*14/15 ?>" src="<?php echo $image_attributes[0] ?>"/></a>
		<?php } ?>
		</div>
		<?php } ?>
